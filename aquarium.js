...
var reach_outboundary = function(p, circle) {
    if(p.dist_from(circle.p) < circle.r){
        return false;
    }
};
...
var Wall = function(p, r) {
    this.p = p;
    this.r = r;
};

// World object contains many agents and walls and food and stuff
var util_add_box = function(circle, x, y, r) {
    circle.push(new Wall(new Vec(x,y), 300));
};
...
var World = function() {
    ...
    this.walls = [];
    var pad = 0;
    util_add_box(this.walls, this.W/2, this.H/2, 300);
    /*
    ...
    */
    ...
    for(var k=0; k<200; k++) {
        for(;;){
            ...
            if(this.walls[0].p.dist_from(new Vec(x, y)) < this.walls[0].r){
                break;
            }
        }
...
World.prototype = {
    ...
    stuff_collide_: function(p1, p2, check_circle, check_items) {
        var minres = false;

        // collide with walls
        if(check_circle) {
            var wall = this.walls[0];
            var res = reach_outboundary(p2, wall);
            if(res) {
    ...
    tick: function() {
        ...
        for(var i=0,n=this.agents.length;i<n;i++) {
            ...
            a.v.x *= 0.95; a.v.y *= 0.95;  // 가만히 놔두면 점점 느려진다. 그 가속이 0.95인듯.
            a.p.x += a.v.x; a.p.y += a.v.y;  // 위치에 속도를 더한다. 한마디로 위치변경
            ...
            if(a.p.dist_from(this.walls[0].p) > this.walls[0].r){
                if(a.p.x > this.walls[0].p.x) {
                    a.v.x -= 10;
                }
                else{
                    a.v.x += 10;
                }
                if(a.p.y > this.walls[0].p.y){
                    a.v.y -= 10;
                }
                else{
                    a.v.y += 10;
        ...
        for(var i=0,n=this.items.length;i<n;i++) {
            ...
            if(it.p.dist_from(this.walls[0].p) > this.walls[0].r){
                it.v.x *= -1;
                it.v.y *= -1;
            }
        ...
        if(this.items.length < 200 && this.clock % 10 === 0 && randf(0,1)<0.25) {
            for(;;){
                ...
                if(this.walls[0].p.dist_from(new Vec(newitx, newity)) < this.walls[0].r){
                    break;
                }
            }
